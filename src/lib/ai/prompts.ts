...
- verkaeufer: data { name: string, legalForm: string, registeredOwnersGrundbuch: string[], authorizedRepresentatives: string[], representationProofProvided: bool, missingProofs: string[] }
  (HINWEIS: Trage bei "name" den vollständigen Namen oder die Firma ohne Höflichkeitsanrede "Herr/Frau" ein (z.B. "Dr. Elena Rostova" oder "Bürger & Partner GbR"). Handeln natürliche Personen im eigenen Namen, setze legalForm ZWINGEND auf "natürliche Person", representationProofProvided: true und missingProofs: []. Werden sie in einem vorliegenden Vertrag oder einer Urkunde als Verkäufer ausgewiesen, ist der Status VERIFIED, es sei denn, ein beiliegender Grundbuchauszug weist abweichende Eigentümer aus. HANDELT ES SICH UM EINE GESELLSCHAFT (GbR, GmbH, UG etc.): Liegt kein amtlicher Registerauszug vor (z.B. eGbR-Gesellschaftsregisterauszug oder HRB), setze representationProofProvided ZWINGEND auf false, führe den fehlenden Nachweis in missingProofs auf und setze den Status auf 'NEEDS_REVIEW'.)
...
- grundbuch: data { amtsgericht: string, grundbuchBezirk: string, blatt: string, standDatum: string, isCurrent: bool }
  (HINWEIS: Werden Grundbuchbezirk und Blattnummer aus einer vorliegenden notariellen Urkunde oder einem Kaufvertrag eindeutig entnommen und liegt kein unvollständiger Teilauszug vor, ist der Status VERIFIED.)
