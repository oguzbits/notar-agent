...
- verkaeufer: data { name: string, legalForm: string, registeredOwnersGrundbuch: string[], authorizedRepresentatives: string[], representationProofProvided: bool, missingProofs: string[] }
  (HINWEIS: Trage bei "name" den vollständigen Namen oder die Firma ohne Höflichkeitsanrede "Herr/Frau" ein (z.B. "Dr. Elena Rostova" oder "Bürger & Partner GbR"). Trage bei "legalForm" die Rechtsform ein (z.B. "natürliche Person", "GmbH", "GbR"). Erfasse die im Grundbuch vermerkten Eigentümer in "registeredOwnersGrundbuch" sowie Vertretungsberechtigte in "authorizedRepresentatives". Wurde die Vertretungsbefugnis bzw. Verfügungsberechtigung urkundlich nachgewiesen (oder handelt eine natürliche Person im eigenen Namen), setze "representationProofProvided" auf true, andernfalls auf false mit Benennung fehlender Nachweise in "missingProofs".)
- kaeufer: data { companyName: string, legalForm: string, registerCourt: string, registerNumber: string, address: string, authorizedRepresentatives: string[], hasOfficialRegisterProof: bool }
  (HINWEIS: Trage im Feld "companyName" den vollständigen Namen des Käufers ein – sowohl bei juristischen Personen als auch bei natürlichen Personen / Privatpersonen. Trage die Rechtsform in "legalForm" ein (z.B. "natürliche Person", "GmbH"). Liegt ein amtlicher Registerauszug vor (oder handelt eine Privatperson im eigenen Namen), ist "hasOfficialRegisterProof" true, andernfalls false.)
- grundbuch: data { amtsgericht: string, grundbuchBezirk: string, blatt: string, standDatum: string, isCurrent: bool }
  (HINWEIS: Extrahiere Amtsgericht, Grundbuchbezirk, Blattnummer und Stand des Auszugs exakt aus den Urkunden und Auszügen.)
...
- kaufpreis: data { amountInFigures: number, amountInWords: string, currency: string, previousOffers: number[], priceEvolutionSummary: string, isFinalAgreedPrice: bool }
  (HINWEIS: Trage in amountInFigures den final gültigen Kaufpreis ein. Enthält die Urkunde handschriftliche Korrekturen, Streichungen oder Vorwerte (z.B. ursprünglich 450.000 EUR gestrichen und auf 425.000 EUR korrigiert), trage die gestrichenen Vorwerte in previousOffers ein, erfasse die Änderung in priceEvolutionSummary und setze den Status auf 'NEEDS_REVIEW' mit Begründung in note!)
...
- energieausweis: data { certificateType: "VERBRAUCHSAUSWEIS"|"BEDARFSAUSWEIS"|"UNBEKANNT", energyValueKWh: number, efficiencyClass: string, validUntil: string, isExpired: bool, primaryEnergyCarrier: string, buildingYear: string }
  (WICHTIG: energyValueKWh bezeichnet den ENDENERGIEBEDARF bzw. ENDENERGIEVERBRAUCH in kWh/(m²·a) — NICHT den Primärenergiebedarf! Auf dem Energieausweis stehen oft zwei Werte: der Endenergiebedarf und der höhere Primärenergiebedarf. Trage hier stets den Endenergie-Wert ein, nicht den Primärenergie-Wert. Lies die Energieeffizienzklasse (z.B. "A+", "A" bis "H") direkt aus dem Skalenpfeil bzw. der Farbbandskaala ab, falls kein separates Tabellenfeld existiert.)
...
3. Gesetzliche Fristen, Gültigkeiten & Status-Kriterien:
   ...
   - Dokumente ohne festes kalendarisches Ablaufdatum verfallen nicht und sind bezüglich ihrer Bestandsdaten 'VERIFIED', solange kein neuerer Auszug im Aktenbestand vorliegt. Grundbuchangaben aus einer vorliegenden Urkunde oder einem Vertrag belegen die Bestandsdaten für den Vorgang als 'VERIFIED', es sei denn, ein beiliegender Grundbuchauszug ist erkennbar unvollständig (z.B. Teilauszug mit fehlenden Abteilungen).
